...
1. It's Free
2. Unlimited customization via packages
* I need a program that has a wide variety of tools to match the wide variety of research interests I have, analytical tools I need, and data sources I deal with
* Descriptives, Regression family, Generalized estimating equations, Path analyses, Latent variable analyses, Latent class analysis, Text analyses and topic modeling, Questionnaire data, Complex survey data, Experimental outcome measures, Perceptual measures
3. You can make beautiful, highly customizable graphs
4. You can work with multiple data files at once
5. You can work with multidimensional data structures beyond table formats
6. You can do variety of other things a programming language allows
* Scraping data from websites/databases, Web-based shiny apps and dashboards, It's easy to create markdown documents, It's easy to create tutorials, You can build packages to share project information, It's really easy to interface with Github
7. R has a very helpful community of users
8. My colleagues in psychology, education, cognitive science, health, etc. use R
9. R is used outside of academia, and knowing R is a marketable skill
10. I like the programming language, it has an internal logic that is readable, and has virtually no limits on it’s application

Things I like about Stata:

1. The learning curve for Stata is less steep
2. Stata is really good for econometric analysis and better than R for working with complex survey data
...
-   Integer - discrete quantitative values with no decimal values (1, 55, 100)
...
Using the base R `install.packages()` function will signal to R to find the package online, download it, and install it for use in your R system. For the **rstatix** packages, that would look like this: `install.packages("rstatix")`.

To "load" the package, you can use the base R `library()` function. Then all of the functions within the package will be available to you. You only need to load a package once during an R session. Loading the **rstatix** packages would look like this: `library(rstatix)`.

As an example of the convenience and functionality that packages can offer,
...
convenient ways to get all of those summary statistics with other packages. For example, both the **rstatix** package and the **psych** package contain a single function for returning all of those summary values.

Here is an example of calculating descriptive statistics summary with base R functions vs. the `get_summary_stats()` function from the **rstatix** package:
...
are designed to provide more intuitive methods for conducting various data science tasks in R. This tutorial will incorporate tidyverse-style coding because it can ease the learning curve when it comes to getting started in R. 

Some tidyverse packages we will use include **ggplot2**: a graphing package,
...
Conducting data analyses in R requires loading your data set as
an object in the R Global Environment. Unlike some programs such as
...
functions for reading in different types of data files have many arguments for telling R how to interpret the file. We won't deal with those here, suffice to say it's important to (a) know what's in the file you are reading into R and (b) check that the file came in the way you wanted it to.
...
In this section, we will use the `select()`, `filter()`, and `mutate()` functions from the **dplyr** package to demonstrate how to select variables from a dataframe for analysis, filter a dataframe according to specific values of a variable, and how to create and add new variables to a dataframe, respectively. 

Given that these functions are from a package within the tidyverse ecosystem, they each can be used with what is referred to as a "pipe operator": `%>%`.
The pipe operator allows you to write code that can be more intuitive to interpret since it can help you visually track the steps in your code.

For these exercises, we will use a dataset provided in this
tutorial package called `DescriptiveData`. This dataset contains four
variables meant to represent characteristics of singers in a choir: (a) `Age` — a numeric variable (i.e., ratio measure), (b) `Years.experience.in.singing.ensemble` - an integer variable (i.e., ratio measure), (c) `Sex` - self-reported biological sex at birth, a two-level factor variable (i.e., nominal measure), and (d) `Preference.for.rehearsal.frequency` - a five-level ordered factor variable (i.e., ordinal measure). This particular dataset is used to illustrate analyses in "Chapter 4: Descriptive Analysis" of the Miksza & Elpus text, [*Design and Analysis for Quantitative Research in Music Education*](https://www.amazon.com/Design-Analysis-Quantitative-Research-Education/dp/0199391904).
...
First we start with the dataframe object then use the pipe `%>%` to indicate that we want to pass the data to another  function, then we call the name of the variable we want to select within the `select()` function, and then we can call a function that we would like to use to act upon the selected variable - note that you don't need to actually write the name of the variable in the final function.
...
`max()` function to identify the largest `Years.experience.in.singing.ensemble` among the participants:
...
Again, we start with the dataframe object then use the pipe `%>%` to indicate that we want to pass the data to the following function, then we call the name of the variable we want to use to to filter our data within the `filter()` function. You can then continue with another pipe to act upon the filtered data with additional functions, or you could create a new dataset from the filtered data assigning the entire line of code to a newly named object.
...
Here is an example of filtering the data so that only those indicating `Sex` as Male are in the newly filtered data - note that quirks in this code include that the "equals" operator requires two signs `==`, and that when you work with levels of factor variables, they need to be presented in quotes, and that the variable names and category labels are case-sensitive.
...
`count()` function to filter the data so that only those younger than `Age`
...
to specify a name for our new variable and then a formula for how create the new variable.
...
argument within the `proportion()` function of `margin = 1` or *by column* 
with `margin = 2`.
...
Examining bivariate correlations is also a common analysis task. As with all 
things in R, there are many, many ways to calculate correlation coefficients and 
a variety of packages with correlation functions. The **rstatix** package has 
several useful functions for calculating correlations. An additional element of
flexibility in all of the rstatix correlation functions is the ability to specify 
among several common types: Pearson, Spearman, or Kendall.
...
There is also a useful function in the **rstatix** package for creating correlation 
matrices `cor_mat()`. Since there are only two interval/ratio level variables in 
this example dataset, it would be inappropriate to demonstrate a Pearson correlation 
matrix of more than two variables.

It could also be helpful to be able to examine correlations according to sub-groups. 
This code first filters the `Sex` variable so only those who reported their biological 
sex are considered in the analyses, then it will result in the Pearson correlation 
between `Age` and `Years.experience.in.singing.ensemble` broken out by participants' 
...
visualizing data are vast. In this section, we will again use the
...
visualization. The primary developer of **tidyverse** packages and functions, 
Hadley Wickham, also developed **ggplot2**. The most basic elements or "layers" 
of a plot rendered with **ggplot2** include: (a) a dataframe, (b) the designation 
of "aesthetics" (e.g., the mapping of variables to axes and aesthetic
features), and (c) a "geom" (e.g., a shape for the plot such as
histogram, boxplot, barplot, scatterplot, and so on).
...
Creating a histogram is critical for examining the distribution of a
quantitative variable. Here is an example of code for creating a
histogram that follows the generic form described above for the `Age` variable 
from the example `DescriptiveData` dataset.
...
Use the code space below to try and code a histogram for the `Years.experience.in.singing.ensemble` variable:
...
variable. Note that the values on the y axis are not informative and refer
to the width of the box. You'll see how to remove them in the next section of 
this tutorial.
...
Use the code space below to try and code a boxplot for the `Years.experience.in.singing.ensemble` variable:
...
Note that the axis labels are very crowded in this plot. We will look at making plot customizations to address issues like that in the next section.
...
measurements of school children: (a) `mathtestk` - kindergarten students'
standardized math test scores, a numeric variable (i.e., interval
measure), (b) `mathtest1` - those same students' 1st grade standardized math test
scores, a numeric variable (i.e., interval measure), (c) `urbanicity` - the urbanicity
of the students' school (i.e., city, suburb, town, rural), a factor variable 
(i.e., nominal measure), (d) `music` - whether they participated in school music class 
(i.e., No Music, Yes Music), a factor variables (i.e., a nominal variable), 
(e) `ses` - a quantitative indiactor of students' socioeconomic status, and (f) a set of "dummy" variables that indicate "Yes" or "No" for each urbanicity 
category - factor variables. This particular dataset is used to illustrate analyses 
in "Chapter 10: Regression" of the Miksza & Elpus text, [*Design and Analysis for Quantitative Research in Music Education*](https://www.amazon.com/Design-Analysis-Quantitative-Research-Education/dp/0199391904).
...
arguments that control outline and fill colors, point size, point shape, line thickness, linetype,degree of transparency, geom dimensions, and so on. Various aesthetic mappings will be demonstrated in the sample code presented below.
...
specifying labels in quotes with arguments in the `labs()` function such as: `x = `, `y = `,  `title = `, `subtitle = `, or `caption = `.
...
the range of values in a plot. You can customize x and y axes in `ggplot()` by adding additional functions such `+` `scale_x_continuous()` and `+` `scale_y_continuous()`. 
You can specify the range of values to be included by inserting the arguments `limits =` into the previous functions, followed by a vector of two values representing the desired minimum and maximum, such as, `+` `scale_x_continuous(limits = c(1, 100))` - that would create an x axis bound by 1 and 100.

If, say, you'd like to customize the axis ticks that are shown between the axis 
minimum and maximum values, then you could also insert the argument `breaks =` into 
the `scale_x_continuous()` function, followed by a vector of values
...
The general "look" of a plot can be controlled with a number of arguments that 
can be specified within a `+` `theme()` layer. Take a look at the online 
...
the complete theme is typically appended to the `ggplot()` function. For example, 
to use what is referred to as a minimalist style in `ggplot()`  you could add a 
layer that looks something like: `+` `theme_minimal()`.

### *Histogram*

Here is code for a histogram depicting the distribution of the students' 1st grade standardized math test score variable.
...
Use this code space to customize your own histogram of `mathtest1` scores.
...
Here is code for a barplot depicting `urbanicity` variable. Note that the labels 
for each bar are taken directly from the factor variable levels. You can change those by relabeling the levels of factor variable outside of the `ggplot()` function using the `labels = ` argument in the `factor()` function.
...
two variables, you can also specify the color scheme with more specific code.
In this example, the `scale_fill_manual()` function is used to adjust the look
of the `fill =` aesthetic. You will find similar functions for other aesthetic 
mappings (`scale_color_manual()`, `scale_linetype_manual()`, etc.).
...
in this section, it demonstrates including a variety of arguemnts that can be used for specific theme specifications rather than a complete theme.
...
We will continue to use the `InferentialTestData` dataset described in the previous section for the examples in this section. To keep things relatively simple, we will also restrict our analyses to the use of the base R linear model function (i.e., `lm()`) and  functions available in the **rstatix** package described earlier. 

In addition, all of the functions that will be demonstrated in the remaining sections can take a formula as the argument for specifying the test you would like to conduct. As such, we will begin with a quick introduction to formula notation.

### *Using Formula Notation*

For the analyses demonstrated in the next section, the formula notation will involve specifying a dependent variable (i.e., an outcome, `y`) and independent variables (i.e., predictors, `x`). The tilde, `~`, symbol is used to separate the dependent from the independent variables. Therefore, a basic analysis of the influence of one independent variable on one dependent variable would look like this:
...
It's worth taking a moment to mention that in addition to base R functions, there are many packages that have functions for doing the relatively simple analyses presented in the next sections. 
...
Many of the statistical tests arts education researchers wish to use can be executed with some generalized form of the **general linear model**. For example, correlation, t-tests, ANOVAs, and regression analyses can all be interpreted as an instantiation of the general linear model. All of the objectives of the statistical tests mentioned in the previous sentence can be achieved with the single base R function `lm()`, which is for fitting linear models.

The `lm()` function is quite simple to use. It involves specifying an argument that is a formula with variable names to indicate dependent and independent variables (e.g., `outcome_var ~ predictor_var`) and then specifying an argument indicating the dataframe in which those variables can be found (e.g., `data = your_dataframe`). 
...
Use the code space below to test whether participants' socioeconomic status, `ses` (i.e., a dependent variable in this case, `ses`) varies as a function of whether participants were in music or not, `music` (i.e., an independent variable, `music`).
```{r independent_tuser, exercise = TRUE, exercise.eval = FALSE}
```

### *Effect Size for Comparing Two Independent Samples*

Calculating the effect size for the difference between two independent means (i.e., Cohen's d) involves similar code with the `cohens_d()` function:
```{r cohen_d, exercise = TRUE, exercise.eval = FALSE}
InferentialTestData %>% cohens_d(mathtest1 ~ music)
```

The same options to arguments described above can be made in the `cohens_d()` function if the analyses involved independent samples with unequal variance (`var.equal = FALSE`) or paired samples (`paired = TRUE`). Similarly, an argument specifying `hedges.correction = TRUE` can be added if Hedge's G is preferred.

Use the code space below to determine the effect size of the difference in participants' socioeconomic status, `ses`, as a function of whether participants were in music or not, `music`.
...
Use the code space below to run a similar test, but with 1st grade standardized math test scores, `mathtest1` as the dependent variable.
...
Here is an example of a test of whether 1st grade standardized math test scores vary as a function of (a) music participation - `music`, (b) urbanicity - `urbanicity`, or (c) whether the effect of music participation depends upon students' urbanicity, that is, the interaction of `music` and `urbanicity`. 
...
All of the ANOVA examples so far have involved between-subjects factors, exclusively. Specifying models with within-subjects factors is a bit more complicated in R. Exploring the options for such models is a bit beyond the scope of this tutorial.
...
The non-parametric analogs of the t-test and ANOVA for nominal or ordinal outcome data are similarly accessible in the **rstatix** package. Use the `?function_name()` help call on the tests below to see what options are avaialble:
