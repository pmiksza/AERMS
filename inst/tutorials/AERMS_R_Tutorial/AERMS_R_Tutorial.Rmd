...
R recognizes similar types of variables, but has slightly different
...
variable while retaining the original.
...
to as "base" R. However, you can easily expand the functionality of R by
...
described above. We will add the traditional "whiskers" to this version with the
additional argument of `staplewidth = ` within the `geom_boxplot()` function and
eliminate the nuisance information on the x axis by specifying `breaks = NULL` in the `scale_x_continuous()` function.
...
ggplot(InferentialTestData, aes(x = Preference.for.rehearsal.frequency)) + geom_bar()
```

```{r barchart_dodgeDetail, exercise = TRUE, exercise.eval = FALSE}
ggplot(InferentialTestData, aes(x = Preference.for.rehearsal.frequency,
                            fill = Sex)) + geom_bar(position = "dodge")
```

### *Scatterplot*

```{r scatterplot1Detail, exercise = TRUE, exercise.eval = FALSE}
ggplot(InferentialTestData, aes(x = Age,
                            y = Years.experience.in.singing.ensemble)) + 
  geom_point() +
  geom_smooth(method = "lm")
```
